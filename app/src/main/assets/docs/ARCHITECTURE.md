...
Gamified security progression. Eleven nodes in a dependency tree, rooted at App PIN. Shield tier (None → Bronze → Silver → Gold → Cyan) derived from node count. Avatar frame visible to all Trusted contacts. Gold = all 10 in-app nodes; Cyan = Gold + Device Owner (external ADB setup, the 11th node).
...
Screens include: ChatList, Chat, GroupChat, SOS, DeviceControl, Map/Radar, Settings, Lock, Profile, Contact detail, SecureNotes, Vault, Security (skill tree), Help (in-app docs), and various settings screens.
