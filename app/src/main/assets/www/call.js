...
const processCommand = (function () {
            ...
            switch (command.type) {
                case "capabilities":
                    ...
                    try {
                        ...
                        console.log(e);
                        // Surface the ACTUAL exception text — this catch
                        // fires for ANY failure (no track, no camera
                        // available, getUserMedia rejected, etc.), not
                        // just permissions. Hardcoding "no permissions"
                        // led to user confusion when perms WERE granted.
                        var msg = (e && (e.message || e.name)) || String(e);
                        resp = { type: "error", message: "capabilities: " + msg };
                        break;
                        localStream = new MediaStream();
    ...
    function setupRemoteStream(call) {
        ...
        pc.ontrack = (event) => {
            ...
            try {
                ...
                // Aegis: also attach the track directly to remoteStream
                // so the <video> element renders as soon as frames
                // arrive. Upstream's design keeps the stream empty
                // until the bytes-received "unmute" heuristic trips —
                // a Safari workaround — but on Chromium Android WebView
                // that heuristic often never fires for the peer's
                // video track even when frames are arriving, leaving
                // the remote video black. Add directly + dedupe in
                // case addRemoteTracksWhenUnmuted runs later.
                const stream = (mediaSource == CallMediaSource.Mic || mediaSource == CallMediaSource.Camera)
                    ? call.remoteStream : call.remoteScreenStream;
                if (stream && !stream.getTracks().includes(track)) {
                    stream.addTrack(track);
                }
                // Aegis: ATTACH stream + FORCE VISIBILITY immediately for
                // the camera transceiver. The peerMediaSources.camera
                // flag (and therefore the <video>.visibility flip in
                // changeLayout) is gated on the bytes-received heuristic
                // detecting growth over 3 consecutive seconds — that
                // heuristic frequently misses on Chromium Android
                // WebView even when frames ARE arriving, so the remote
                // <video> stays hidden and the user just sees their
                // own local PIP. The track itself is live, the receiver
                // is decoding fine — only the visibility flag is wrong.
                // Set it directly here from the ontrack signal and let
                // the bytes-received listener still run as the source
                // of truth for genuine peer mute/unmute mid-call.
                if (mediaSource == CallMediaSource.Camera && track.kind === "video") {
                    try {
                        ...
                        const videos = getVideoElements();
                        if (videos) {
                            if (!videos.remote.srcObject || videos.remote.srcObject !== call.remoteStream) {
                                videos.remote.srcObject = call.remoteStream;
                            }
                            videos.remote.style.visibility = "visible";
                            videos.remote.play().catch((e) => console.log(`remote play: ${e.message}`));
                        }
                        sendMessageToNative({ resp: { type: "peerMedia", source: CallMediaSource.Camera, enabled: true } });
                        console.log("ontrack: forced remote video visibility on camera transceiver");
                    } catch (e) {
                        console.log(`ontrack force-visibility error: ${e.message}`);
                    }
                }
    ...
    */
    ...
    async function getLocalMediaStream(mic, camera, facingMode) {
        ...
        try {
            return await navigator.mediaDevices.getUserMedia(callMediaConstraints(mic, camera, facingMode));
        }
        catch (e) {
            // Aegis resilience: a combined mic+camera getUserMedia is
            // ATOMIC in Chromium/WebView — if EITHER device can't open,
            // the whole promise rejects, so a mic that won't start
            // ("Could not start audio source" / NotReadableError —
            // common on Android when another process holds the mic, an
            // OEM audio HAL hiccups, or RECORD_AUDIO isn't granted)
            // killed the VIDEO too. The caller's Android branch then
            // rethrows ("all streams should be present"), aborting the
            // whole call — the "remote live camera shows nothing, error
            // about the audio device" report. For the live-camera / SOS
            // stream surface the video IS the point and audio is a
            // bonus, so degrade instead of failing closed: retry
            // video-only, then mic-only, returning whatever the device
            // can actually open. A one-track stream still establishes
            // the call; an exception establishes nothing.
            if (mic && camera) {
                const reason = (e && (e.name || e.message)) || String(e);
                console.log("getUserMedia(mic+camera) failed (" + reason + ") — retrying video-only");
                try {
                    return await navigator.mediaDevices.getUserMedia(callMediaConstraints(false, camera, facingMode));
                }
                catch (e2) {
                    const reason2 = (e2 && (e2.name || e2.message)) || String(e2);
                    console.log("getUserMedia(video-only) failed (" + reason2 + ") — retrying audio-only");
                    return await navigator.mediaDevices.getUserMedia(callMediaConstraints(mic, false, facingMode));
                }
            }
            throw e;
        }
    ...
    function callMediaConstraints(mic, camera, facingMode) {
        return {
            audio: !mic
                ? false
                : {
                    // Aegis: WebRTC's default audio constraints leave
                    // AEC/NS/AGC off, which produced an escalating
                    // feedback loop when the speaker's output bled into
                    // the mic. These are the standard VoIP defaults
                    // every working app sets.
